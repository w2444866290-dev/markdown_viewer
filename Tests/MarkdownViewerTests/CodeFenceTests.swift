...
extension StylerSuites {
    ...
    struct CodeFenceTests {
        ...
        @Test func codeFenceAndLanguageLabel() {
            ...
            guard let body = requireIndex(ts, of: "let x = 1") else { return }
            ...
            guard let lang = requireIndex(ts, of: "swift") else { return }
        ...
        @Test func cardContinuityAcrossBlankLine() {
            ...
            for i in 0..<n where !StylerProbe.boolAttr(ts, .mvCodeBlock, i) {
                fail("mvCodeBlock must be contiguous; missing at char \(i)")
            }
        }
