...
struct MvNonBodyMapTests {
    ...
    @Test func globalMvNonBodyMap() {
        ...
        // Heading `#`.
        expectChar(ts, StylerProbe.index(of: "# Heading", in: ts), "heading #")
        ...
        // Blockquote marker `>`.
        expectChar(ts, StylerProbe.index(of: "> A quoted", in: ts), "quote >")

        // List markers `- ` and `1. `.
        let dash = StylerProbe.index(of: "- list item alpha", in: ts)
        expectChar(ts, dash, "list dash")
        expectChar(ts, dash + 1, "list marker space")
        let num = StylerProbe.index(of: "1. numbered beta", in: ts)
        expectChar(ts, num, "ordered 1")
        expectChar(ts, num + 1, "ordered dot")

        // Table pipe + separator.
        expectChar(ts, StylerProbe.index(of: "| Col", in: ts), "table pipe")
        expectNonBody(ts, substring: "|-----|-----|", true)

        // Fence markers + language label.
        let open = StylerProbe.index(of: "\(fence)lang", in: ts)
        ...
        let hr = StylerProbe.index(of: "\n---\n", in: ts) + 1
        for i in hr..<(hr + 3) { expectChar(ts, i, "hr dash \(i)") }

        // Link/image bracket syntax.
        expectChar(ts, StylerProbe.index(of: "[label]", in: ts), "link [")
        expectChar(ts, StylerProbe.index(of: "![alt]", in: ts), "image !")
    }

    private func expectChar(_ ts: NSTextStorage, _ i: Int, _ what: String,
                            sourceLocation: SourceLocation = #_sourceLocation) {
        if !StylerProbe.isNonBody(ts, i) {
            fail("\(what): expected mvNonBody at \(i)", sourceLocation: sourceLocation)
        }
    }
    ...
    private func expectDelimiters(_ ts: NSTextStorage, around sub: String, left: Int, right: Int,
        ...
        guard r.length > 0 else { fail("\(sub) not found", sourceLocation: sourceLocation); return }
        for i in 0..<left where !StylerProbe.isNonBody(ts, r.location + i) {
            fail("\(sub) left delim \(i) should be non-body", sourceLocation: sourceLocation)
        }
        for i in 0..<right where !StylerProbe.isNonBody(ts, r.location + r.length - 1 - i) {
            fail("\(sub) right delim \(i) should be non-body", sourceLocation: sourceLocation)
        }
        for i in left..<(r.length - right) where StylerProbe.isNonBody(ts, r.location + i) {
            fail("\(sub) interior \(i) should be body", sourceLocation: sourceLocation)
        }
    }
}
